...
fn main() -> ! {
    ...
    cfg_if! {
        ...
        } else if #[cfg(hubris_chip = "STM32H753")] {
...
impl Stm32Sys for Server {
    fn set_pin_output(&mut self, _: Meta, port: Port, pin: u8) -> Result<(), ReplyFaultReason> {
        ...
        gpio.moder()
            .modify(|v| v.set_moder(convert_pin(pin), Moder::OUTPUT));
        Ok(())
    }
    ...
    fn set_pin_alternate_mode(
        ...
        gpio.afr(if pin < 8 { 0 } else { 1 })
            .modify(|r| r.set_afr(pin & 0b111, af));
    ...
    fn set_pin_input(&mut self, _: Meta, port: Port, pin: u8) -> Result<(), ReplyFaultReason> {
        let gpio = get_port(port)?;
        gpio.moder()
            .modify(|v| v.set_moder(convert_pin(pin), Moder::INPUT));
        Ok(())
    }
    ...
    fn set_pins_output(&mut self, _: Meta, port: Port, mask: u16) -> Result<(), ReplyFaultReason> {
        let gpio = get_port(port)?;
        gpio.moder().modify(|w| {
            for i in 0..16 {
                if mask & (1 << i) == 0 {
    ...
    fn read_pins(&mut self, _: Meta, port: Port) -> Result<u16, ReplyFaultReason> {
        let gpio = get_port(port)?;
...
fn get_port(port: Port) -> Result<stm32_metapac::gpio::Gpio, ReplyFaultReason> {
    match port {
        ...
        #[cfg(hubris_chip = "STM32H753")]
        Port::F => Ok(stm32_metapac::GPIOH),
        #[cfg(hubris_chip = "STM32H753")]
        Port::H => Ok(stm32_metapac::GPIOH),
        // TODO Check remaining for STM32H753
